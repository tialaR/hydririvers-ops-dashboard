...
export function OwnedCargoDesktopFoundation({ cargoes }: Props) {
  ...
  return <main className={`${styles.root} ${visualFixtureEnabled ? styles.fixtureRoot : ''}`} data-testid="m01-desktop-foundation">
    ...
    <header className={styles.header}>{visualFixtureEnabled ? <><div/><div className={styles.fixtureHeaderControls}><span/><span/><span/><i/></div></> : <><div><strong>{t('myCargoes.title')}</strong><small>{t('myCargoes.desktop.workspace')}</small></div><div><Search size={17}/><Bell size={17}/><span className={styles.avatar}>{initials}</span></div></>}</header>
    <section className={styles.master} aria-label={t('myCargoes.desktop.masterAria')}>
      <div className={styles.masterTitle}>{visualFixtureEnabled ? <><div><h1>Minhas Cargas</h1></div><small className={styles.fixtureAttentionSummary}>2 exigem atenção</small><span className={styles.fixtureFilterButton} aria-hidden="true"><i/><i/><i/></span></> : <><div><h1>{t('myCargoes.title')}</h1><small>{t('myCargoes.resultsCount',{count:visible.length})}</small></div><button type="button" aria-label={t('myCargoes.desktop.moreActions')}>•••</button></>}</div>
